...
function elgg_eventcloud_page_handler($page) {
    monitor_gatekeeper();
	switch ($page[0]) {
		default:
			$title = elgg_view_title(elgg_echo('tags:site_cloud'));
			...
			echo elgg_view_page(elgg_echo('events:site_cloud'), $body);
			break;
	}
}
...
function elgg_tags_init() {
	...
	elgg_register_page_handler('events', 'elgg_eventcloud_page_handler');
}
