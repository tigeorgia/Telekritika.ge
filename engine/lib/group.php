...
/**
 * Adds a channel tool option
 *
 * @see remove_channel_tool_option().
 *
 * @param string $name       Name of the channel tool option
 * @param string $label      Used for the channel edit form
 * @param bool   $default_on True if this option should be active by default
 *
 * @return void
 * @since 1.5.0
 */
function add_channel_tool_option($name, $label, $default_on = true) {
	global $CONFIG;

    if (!isset($CONFIG->channel_tool_options)) {
        $CONFIG->channel_tool_options = array();
    }

    $channel_tool_option = new stdClass;

    $channel_tool_option->name = $name;
    $channel_tool_option->label = $label;
    $channel_tool_option->default_on = $default_on;

    $CONFIG->channel_tool_options[] = $channel_tool_option;
...
/**
 * Removes a channel tool option based on name
 *
 * @see add_channel_tool_option()
 *
 * @param string $name Name of the channel tool option
 *
 * @return void
 * @since 1.7.5
 */
function remove_channel_tool_option($name) {
	global $CONFIG;
    ...
    foreach ($CONFIG->channel_tool_options as $i => $option) {
        if ($option->name == $name) {
            unset($CONFIG->channel_tool_options[$i]);
        }
    }
