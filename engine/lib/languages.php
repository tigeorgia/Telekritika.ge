...
function elgg_echo($message_key, $args = array(), $language = "") {
    ...
    //ADDED BY BRANDON TO CREATE TRANSLATION TODOs
	if ($string = $CONFIG->translations["ka"][$message_key]) {
	    $fail = $completefail = false;        
    } elseif ($string = $CONFIG->translations["en"][$message_key]) {
        $fail = true;
    } elseif ($string = $message_key){
        $completefail = true;
    }
    if($CONFIG->translation_todo_recording && ($completefail || $fail)){
        write_custom_key($message_key, $string);           
    }
    ...
    if($CONFIG->translation_todo_shortcut && elgg_is_admin_logged_in())
        $string = "<span class='elgg-echo-string' data-key='$message_key'>$string</span>";        
