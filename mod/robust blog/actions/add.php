<?php
	...
	// notify watchers
		...
		if ($watch != 'no'){
		$watchers = elgg_get_entities_from_relationship(array('blogwatcher', $blog->owner_guid, TRUE));
		foreach($watchers as $watcher){
			$name = $watcher->name;
			$subject = $name . elgg_echo('blog:watch:notify:subject');
			$message = elgg_echo('blog:watch:notify:message');
			notify_user($watcher->guid,$blog->owner_guid,$subject,$message);	
			}
			}
