...
if (!empty($vars['tagcloud']) && is_array($vars['tagcloud'])) {
	...
	foreach ($vars['tagcloud'] as $tag) {
		...
		$url = elgg_get_site_url()."search?q=". urlencode($tag->tag);
        // . "&search_type={$vars['tag_name']}";
    ...
    //echo elgg_get_view_location('tagcloud/extend');
    $type = $vars['alttype'] ? $vars['alttype'] : $vars['subtype'];
	echo "<div data-tagtype=\"{$type}_{$vars['tag_name']}\"class=\"elgg-tagcloud elgg-tagcloud-{$vars['tag_name']}\">$cloud</div>";
}
