...
class hjSegment extends ElggObject {
    ...
    public function getSectionContent($section, $widget = null, $limit = 0) {
        
        if ($widget && elgg_instanceof($widget, 'object', 'widget')) {
            $metadata = array(array('name' => 'widget', 'value' => $widget->guid));
        } else {
            $metadata = null;
        }
        
        $content = hj_framework_get_entities_from_metadata_by_priority('object', $section, $this->owner_guid, $this->guid, $metadata, $limit);
