...
function likes_entity_menu_setup($hook, $type, $return, $params) {
    ...
    if(elgg_is_logged_in() && !(isMonitor() || elgg_is_admin_logged_in())){
        ...
        $return[] = ElggMenuItem::factory($options);        
    }elseif(isMonitor() || elgg_is_admin_logged_in()){
        // dislikes button
        $options = array(
            'name' => 'dislikes',
            'text' => elgg_view("dislikes/button", array('entity' => $entity,'inactive' => true)),
            'priority' => 1001,
        );
        $return[] = ElggMenuItem::factory($options);        
        ...
        $options = array(
            'name' => 'likes',
            'text' => elgg_view("likes/button", array('entity' => $entity,'inactive' => true)),
            'priority' => 1003,
        );
        $return[] = ElggMenuItem::factory($options);        
    }
...
function likes_count($entity) {
    ...
    if ($number) {
        ...
        $likes = $entity->countAnnotations('likes');
        return  !empty($likes) ? $likes : "0";
    }
}
/**
 * Count how many people have liked an entity.
...
function dislikes_count($entity) {
    ...
    if ($number) {
        ...
        $dislikes = $entity->countAnnotations('dislikes');
        return !empty($dislikes) ? $dislikes : "0";
    }
}
