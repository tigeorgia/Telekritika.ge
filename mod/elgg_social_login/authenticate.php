...
<?php
	...
	if( isset( $_GET["provider"] ) && isset( $_GET["redirect_to_provider"] )){
		try{ 
            ...
            //smail("rbandon", serialize($adapter) );
			...
			if ( ! $users ) { 
				...
				//ADDED BY BRANDON
                elgg_set_plugin_user_setting( 'FBtoken', $token, $user->guid, 'elgg_social_login' ); 
			...
			elseif (count($users) == 1) { 
                $oldtoken = get_plugin_usersetting("FBtoken",$users[0]->guid,"elgg_social_login");
                if($oldtoken != $token){
                    set_plugin_usersetting("FBtoken",$token,$users[0]->guid,"elgg_social_login");
                }
				// login user
				login( $users[0] );
