...
function developers_wrap_views($hook, $type, $result, $params) {
	...
	if ($result) {
        global $CONFIG;
        $theview = elgg_get_view_location($view);    
        $context = implode(",", $CONFIG->context);
		$insertvalue = " title=\"{$theview}default/{$view}.php | $context\" ";
        $result = preg_replace('%(<div)|(<form)|(<span)|(<a)|(<ui)|(<li )|(<img)|(<input)|(<select)|(<textarea)%', '\0'.$insertvalue, $result, 1);  
//        $result = "<!-- BEGIN {$context} -->$result<!-- END {$context} -->";
    }
