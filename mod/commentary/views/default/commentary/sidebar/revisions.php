...
if (elgg_instanceof($commentary, 'object', 'commentary') && $commentary->canEdit()) {
	...
	if ($revisions) {
		...
		$load_base_url = "channels/edit/{$commentary->getGUID()}";

		// show the "published revision"
		if ($commentary->status == 'published' || $commentary->status == 'featured') {
			$load = elgg_view('output/url', array(
				'href' => $load_base_url,
				'text' => elgg_echo('commentary:status:'.$commentary->status)
			));
