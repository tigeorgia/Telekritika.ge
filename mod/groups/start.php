...
//    elgg_register_menu_item('site', $item);
     ...
     // When upgrading, check if the ElggSegment class has been registered as this
     // was added in Elgg 1.8
        add_subtype('group', 'channel', 'ElggChannel');

    elgg_register_page_handler('channelsadmin', 'channels_page_handler');
...
//	elgg_register_plugin_hook_handler('entity:icon:url', 'channel', 'channels_icon_url_override');
    ...
    elgg_register_plugin_hook_handler('entity:icon:url', 'channel', 'groups_icon_url_override');
...
if(elgg_is_admin_logged_in()){
    // Set up the menu
    $item = new ElggMenuItem('channels', elgg_echo('channels:edit'), 'channelsadmin/all');
    elgg_register_menu_item('topbar', $item);

	// Register some actions
	$action_base = elgg_get_plugins_path() . 'groups/actions/groups';
    elgg_register_action("groups/editgroup", "$action_base/editgroup.php");
	elgg_register_action("groups/editchannel", "$action_base/editchannel.php");
...
//	add_channel_tool_option('activity', elgg_echo('channels:enableactivity'), true);
//    elgg_extend_view('groups/tool_latest', 'groups/profile/activity_module');
//	elgg_extend_view('channels/tool_latest', 'channels/profile/activity_module');
...
//	elgg_register_plugin_hook_handler('access:collections:write', 'all', 'groups_write_acl_plugin_hook');
 ...
 //   elgg_register_plugin_hook_handler('permissions_check', 'all', 'groups_permissions_check');
...
function channels_page_handler($page) {
	...
	elgg_push_breadcrumb(elgg_echo('channels'), "channelsadmin/all");
    elgg_push_context('channelsadmin');
    
	switch ($page[0]) {
		case 'all':
			groups_handle_all_page('channel');
			break;
		case 'search':
			groups_search_page('channel');
			break;
		case 'owner':
			groups_handle_owned_page($page[1], 'channel');
			break;
		case 'member':
			set_input('username', $page[1]);
			groups_handle_mine_page('channel');
			break;
		case 'invitations':
			set_input('username', $page[1]);
			groups_handle_invitations_page('channel');
			break;
		case 'add':
			groups_handle_edit_page('add', '', 'channel');
			break;
		case 'edit':
			groups_handle_edit_page('edit', $page[1], 'channel');
			break;
		case 'profile':
			groups_handle_profile_page($page[1], 'channel');
			break;
		case 'activity':
			groups_handle_activity_page($page[1], 'channel');
			break;
		case 'members':
			groups_handle_members_page($page[1], 'channel');
			break;
		case 'invite':
			groups_handle_invite_page($page[1], 'channel');
			break;
		case 'requests':
			groups_handle_requests_page($page[1], 'channel');
			break;
	}
}
 ...
 */
function channels_url($entity) {
    $title = elgg_get_friendly_title($entity->name);

    return "channels/{$entity->name}";
}
...
function groups_icon_url_override($hook, $type, $returnvalue, $params) {
	...
	if (isset($group->icontime)) {
		...
		return $CONFIG->wwwroot . "groupicon/$group->guid/$size/$icontime.jpg";
	}

	return $CONFIG->wwwroot . "mod/groups/graphics/default{$size}.gif";
}
...
function groups_activity_owner_block_menu($hook, $type, $return, $params) {
	if (elgg_instanceof($params['entity'], 'group')) {
		if ($params['entity']->activity_enable != "no") {
            ...
            $url = "{$subtype}s/activity/{$params['entity']->guid}";
			$item = new ElggMenuItem('activity', elgg_echo($subtype.'s:activity'), $url);
			$return[] = $item;
		}
	}
...
function groups_entity_menu_setup($hook, $type, $return, $params) {
    ...
    foreach ($return as $index => $item) {
        if (in_array($item->getName(), array('access', 'edit', 'delete')) && !elgg_is_admin_logged_in()) {
            unset($return[$index]);
        }
    }
/*
...
function discussion_init() {
    ...
    add_channel_tool_option('forum', elgg_echo('channels:enableforum'), true);
    elgg_extend_view('groups/tool_latest', 'discussion/group_module');
    elgg_extend_view('channels/tool_latest', 'discussion/channel_module');
...
function discussion_page_handler($page) {
	...
	switch ($page[0]) {
		case 'all':
			discussion_handle_all_page($subtype);
			break;
		case 'owner':
			discussion_handle_list_page($page[1],$subtype);
			break;
		case 'add':
			discussion_handle_edit_page('add', $page[1],$subtype);
			break;
		case 'edit':
			discussion_handle_edit_page('edit', $page[1],$subtype);
			break;
		case 'view':
			discussion_handle_view_page($page[1],$subtype);
			break;
	}
}
...
function discussion_owner_block_menu($hook, $type, $return, $params) {
	if (elgg_instanceof($params['entity'], 'group')) {
		if ($params['entity']->forum_enable != "no") {
			...
			$item = new ElggMenuItem('discussion', elgg_echo('discussion:'.$subtype), $url);
			$return[] = $item;
		}
	}
