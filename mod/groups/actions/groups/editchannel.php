...
$input['autourl_root'] = get_input('autourl_root');
...
$group = new ElggChannel($group_guid); // load if present, if not create a new group
if (($group_guid) && (!$group->canEdit())) {
	register_error(elgg_echo("channels:cantedit"));
...
// Validate create
if (!$group->name) {
	register_error(elgg_echo("channels:notitle"));
...
if (isset($CONFIG->channel_tool_options)) {
	foreach ($CONFIG->channel_tool_options as $group_option) {
...
system_message(elgg_echo("channels:saved"));

$url = "channelsadmin/edit/{$group->getGUID()}";
