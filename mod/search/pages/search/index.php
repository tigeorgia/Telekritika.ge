...
$pattern = $CONFIG->noSearchTypes;

foreach ($types as $type => $subtypes) {    
    if(in_array($type, $pattern)){
        unset($types[$type]);
        continue;
    }
	...
	if (is_array($subtypes) && count($subtypes)) {
		foreach ($subtypes as $subtype) {
            if(in_array($subtype, $pattern)){
                unset($types[$type][$subtype]);
                continue;
            }
...
$sidebody = elgg_view_menu('page', array('sort_by' => 'name'));            
$params['content']['side'][0] = array("body" => elgg_view_module("side", null, $sidebody, array("class" => "")), "class" => "");


unset($CONFIG->menus['page']);
...
$params['content']['main'][] = 
    array(
        "body" => elgg_view_module("search", null, $layout, array("class" => "tk-main-module")), 
        "class" => "toppest"
    );


$finalbody = elgg_view_layout("custom_index", $params);

echo elgg_view_page($title, $finalbody);
