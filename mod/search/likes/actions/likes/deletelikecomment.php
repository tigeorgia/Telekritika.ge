...
if (!empty($comment_info['likes']) && in_array(elgg_get_logged_in_user_guid(), $comment_info['likes'])) {
    unset($comment_info['likes'][(string)elgg_get_logged_in_user_guid()]);
}else{
    system_message(elgg_echo("likes:alreadyunliked"));
    forward(REFERER);    
}
...
system_message(elgg_echo("likes:unlikes"));
