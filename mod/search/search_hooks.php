...
//smail("test",$params['subtype']);
    
    global $CONFIG;
    if(!in_array($params['subtype'], $CONFIG->allowedSearchObjectSubtypes) || $params['subtype'] === 0)return false;
...
//        $sanitised_tagssend.='"' . sanitise_string($tag) . '"';
        $sanitised_tags[] = '"' . sanitise_string($tag) . '"';
    }

    $tags_in = implode(',', $sanitised_tags);

//    smail("tagsin", $tags_in);
//    $tags_in = '"tags"';
//    $params['wheres'][] = "(msn.string IN ($tags_in) AND msv.string = '$query')";
    ...
    foreach ($entities as $entity) {
        ...
        foreach ($search_tag_names as $tag_name) {
            ...
            // the tag labels.
//            if (in_array(strtolower($query), $tags)) {
 ...
 * @return unknown_type
 */
function search_events_hook($hook, $type, $value, $params) {
    $db_prefix = elgg_get_config('dbprefix');
    
    $valid_tag_names = array('events');
 ...
 * @return unknown_type
 */
function search_categories_hook($hook, $type, $value, $params) {
	$db_prefix = elgg_get_config('dbprefix');
    
	$valid_tag_names = array('universal_categories');
...
function search_comments_hook($hook, $type, $value, $params) {
    ...
    $type = ($type == "comments") ? 'generic_comment' : $type;
    $query = sanitise_string($params['query']);
//    $params['annotation_names'] = array('generic_comment');
    $params['annotation_names'] =  array($type);
    ...
    foreach ($comments as $comment) {
        $comment_info = @json_decode($comment->comment, TRUE);
        $thecomment = !empty($comment_info) ? $comment_info['originalvalue'] : $comment->comment;
        if( !empty($comment_info) && strpos($comment_info['originalvalue'], $query) === false ){
            $count = $count - 1;
        }else{
            ...
            $comment_str = search_get_highlighted_relevant_substrings($thecomment, $query);
