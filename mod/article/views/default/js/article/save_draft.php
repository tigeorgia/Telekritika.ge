 ...
 */
?>
elgg.provide('elgg.article');

/*
 * Attempt to save and update the input with the guid.
 */
elgg.article.saveDraftCallback = function(data, textStatus, XHR) {
    if (textStatus == 'success' && data.success == true) {
        var form = $('form[name=article_post]');
        ...
        $(".article-save-status-time").html(d.toLocaleDateString() + " @ " + d.getHours() + ":" + mins);
    } else {
        $(".article-save-status-time").html(elgg.echo('error'));
    }
}

elgg.article.saveDraft = function() {
    ...
    var draftURL = elgg.config.wwwroot + "action/article/auto_save_revision";
    ...
    $.post(draftURL, postData, elgg.article.saveDraftCallback, 'json');
}

elgg.article.init = function() {
    // get a copy of the body to compare for auto save
    oldDescription = $('form[name=article_post]').find('textarea[name=description]').val();
    
    setInterval(elgg.article.saveDraft, 60000);
...
elgg.register_hook_handler('init', 'system', elgg.article.init);